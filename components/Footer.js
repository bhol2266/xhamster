    ...
    return (
        ...
        <div className={`xl:px-[150px] 2xl:px-[250px] font-footer p-8 sm:px-[70px] mx-auto bg-semiblack  ${spinnerLoading ? "hidden" : ""}  mt-10 text-white select-none`}>
            ...
            <Link href='/membership'>
                <button className=" lg:hidden bg-theme_yellow text-semiblack rounded-[22px] font-inter text-center mb-8 mt-4 px-[70px] p-3 m-1 text-sm block_popunder hover:scale-105 transition-transform duration-300 mx-auto block">
                    GET MEMBERSHIP
                </button>
            </Link>
            ...
            <Link href='/membership'>
                <button className="text-white lg:block bg-theme_yellow text-semiblack rounded-[22px] font-inter text-center mb-8 mt-4 px-[70px] p-3 m-1 text-md block_popunder hover:scale-105 transition-transform duration-300 hidden">
                    GET MEMBERSHIP
                </button>
            </Link>
            ...
            <div className="flex flex-wrap  justify-between my-4">
                <div className="w-1/2 lg:w-1/3  mb-4">
                    <p className='font-semibold text-theme_yellow text-[17px] text-left mb-2 lg:text-[20px]'>General</p>
                    <Link passHref={true} href={'/membership'}>
                        <p className=' text-white text-[15px] text-left my-3 w-fit border-b-2 border-transparent hover:border-theme_yellow transition-colors lg:text-[18px]'>Join</p>
                    </Link>
                    <p onClick={() => { setLoginModalVisible(true) }} className='text-white text-[15px] text-left my-3 w-fit border-b-2 border-transparent hover:border-theme_yellow transition-colors lg:text-[18px]'>Member Sign in</p>

                    <p className='text-white text-[15px] text-left my-3 w-fit border-b-2 border-transparent hover:border-theme_yellow transition-colors lg:text-[18px]'>Password Recovery</p>

                    <a rel="nofollow" href='https://theporndude.com' >
                        <p className='text-white text-[15px] text-left my-3 w-fit border-b-2 border-transparent hover:border-theme_yellow transition-colors lg:text-[18px]'>ThePornDude</p>
                    </a>
